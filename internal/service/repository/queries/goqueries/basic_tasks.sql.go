...
const addBasicTask = `-- name: AddBasicTask :exec
...
func (q *Queries) AddBasicTask(ctx context.Context, db DBTX, arg AddBasicTaskParams) error {
	_, err := db.ExecContext(ctx, addBasicTask,
		arg.UserID,
		arg.Text,
		arg.Start,
		arg.Description,
...
const deleteBasicTask = `-- name: DeleteBasicTask :exec
DELETE
FROM single_tasks
WHERE id = ?1
`

func (q *Queries) DeleteBasicTask(ctx context.Context, db DBTX, id int64) error {
	_, err := db.ExecContext(ctx, deleteBasicTask, id)
	return err
}
