...
func (nr *NotificationParamsRepository) Get(ctx context.Context, userID int) (models.NotificationParams, error) {
	op := "notificationParamsRepository.Get: %w"
	params, err := nr.q.GetDefaultUserNotificationsParams(ctx, int32(userID))
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return models.NotificationParams{}, fmt.Errorf(op, serverrors.NewNotFoundError(err, "default notification params"))
		}
...
func (nr *NotificationParamsRepository) Set(ctx context.Context, userID int, params models.NotificationParams) (models.NotificationParams, error) {
